...
def plotting_acc_loss(acc_values: list, loss_values: list, epochs: int) -> None:
    ...
    plt.savefig(f"./Images/acc_loss_gcn2_230.png", dpi=600)
...
def plotting_confusion_matrix(true_label: np.array, pred_label: np.array) -> None:
    ...
    plt.savefig(f"./Images/confusion_matrix_gcn2_230.png", dpi=600)
...
def getting_classification_report(true_label: np.array, pred_label: np.array, set_name: str):
    ...
    with open(f"./Images/report_gcn2_230.txt", "w") as f:
        f.write(set_name)
        f.write("\n")
...
class SyntheticDataset(DGLDataset):
    def __init__(self, attr_edges: pd.DataFrame, attr_properties: pd.DataFrame, attr_features: dict):
...
if __name__ == '__main__':
    ...
    print("segmentating...")
    train_signals_V1, train_signals_II = segmentation_signals(PATH, FILES_TRAIN, 100, 130, 'Train')
    val_signals_V1, val_signals_II = segmentation_signals(PATH, FILES_VAL, 100, 130, 'Train')
