...
def scaling_dataset(signals_II: dict) -> Tuple[np.asarray, np.asarray, np.array]:
    ...
    X = np.asarray(ecg_list).astype(np.float32).reshape(-1, 300, 1)
...
def plot_history_metrics(history: Type[keras.callbacks.History]) -> None:
    ...
    for key, value in history.history.items():
        ...
        plt.title(str(key))
        plt.savefig(f'./images/small_300/{str(key)}.png',
                    dpi=600)
...
def training(X_train: np.asarray, y_train: np.asarray, X_val: np.asarray, y_val: np.asarray,
    ...
    sns.heatmap(confusion_matrix(y_true, prediction),
                ...
                fmt=".0f",
                cmap="rocket_r")
    plt.savefig(f"./images/small_300/confusion_matrix_cnn_small.png", dpi=600)

    with open("./images/small_300/report_cnn_small.txt", "w") as f:
        f.write(classification_report(y_true, prediction, zero_division=0))

    with open('./images/small_300/cnn_small.txt', 'w') as f:
        f.write('TRAIN\n')
...
if __name__ == '__main__':
    ...
    print("segmentating...")
    train_signals_II = segmentation_signals(PATH, FILES_TRAIN, 100, 200, "Train")
    val_signals_II = segmentation_signals(PATH, FILES_VAL, 100, 200, "Train")
