...
def get_beats_features(signals_V1: defaultdict, signals_II: defaultdict) -> dict:
...
def plotting_acc_loss(acc_values: list, loss_values: list, epochs: int) -> None:
    ...
    plt.savefig("./Images7/acc_loss_gcn7_II_V1.png", dpi=600)
...
def plotting_confusion_matrix(true_label: np.array, pred_label: np.array) -> None:
    ...
    plt.savefig("./Images7/confusion_matrix_gcn7_II_V1.png", dpi=600)
...
def getting_classification_report(true_label: np.array, pred_label: np.array, set_name: str):
    ...
    with open("./Images7/report_gcn7_II_V1.txt", "w") as f:
        f.write(set_name)
        f.write("\n")
...
def training(dataset_train: Type[dgl.data.DGLDataset], dataset_test: Type[dgl.data.DGLDataset]) -> None:
    ...
    model = GCN(3, 20, 3)  # (n_nodes_features, n_nodes_hidden_layer, n_classes)
...
if __name__ == "__main__":
    ...
    test_signals_V1, test_signals_II = segmentation_signals(PATH, FILES_TEST, 100, 180, "Test")

    print("sampling...")
    train_signals_V1 = sampling_windows_10_beats(train_signals_V1)
    train_signals_II = sampling_windows_10_beats(train_signals_II)
    test_signals_V1 = sampling_windows_10_beats(test_signals_V1)
    ...
    test_features = get_beats_features(test_signals_V1, test_signals_II)
